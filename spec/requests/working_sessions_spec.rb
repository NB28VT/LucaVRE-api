...
RSpec.describe "WorkingSessions API", type: :request do
  ...
  describe "GET /api/working_sessions/:id" do
    it "returns the requested working session" do
      working_session = create(:working_session)

      get "/api/working_sessions/#{working_session.id}"
    ...
    it "returns a 404 when the working session does not exist" do
      get "/api/working_sessions/does-not-exist"
  ...
  describe "POST /api/working_sessions" do
    it "creates a working session with the given car and track" do
      expect {
        post "/api/working_sessions", params: { working_session: { car_id: "car-1", track_id: "track-1" } }
    ...
    it "ignores unpermitted params via strong params" do
      post "/api/working_sessions", params: {
    ...
    it "returns unprocessable_content and does not persist when validations fail" do
      ...
      expect {
        post "/api/working_sessions", params: { working_session: { car_id: "", track_id: "" } }
  ...
  describe "PATCH /api/working_sessions/:id" do
    ...
    it "returns a 404 when the working session does not exist" do
      patch "/api/working_sessions/does-not-exist", params: { working_session: { car_id: "new-car" } }

      expect(response).to have_http_status(:not_found)
    end
  end

  describe "DELETE /api/working_sessions/:id" do
    it "deletes the working session" do
      working_session = create(:working_session)

      expect {
        delete "/api/working_sessions/#{working_session.id}"
      }.to change(WorkingSession, :count).by(-1)
    ...
    it "returns a 404 when the working session does not exist" do
      delete "/api/working_sessions/does-not-exist"
