...
RSpec.describe "HandlingDeficits API", type: :request do
  describe "GET /api/working_sessions/:working_session_id/handling_deficits" do
    it "returns all handling deficits for the working session serialized with camelCase keys" do
      ...
      get "/api/working_sessions/#{working_session.id}/handling_deficits"
    ...
    it "returns a 404 when the working session does not exist" do
      get "/api/working_sessions/does-not-exist/handling_deficits"
  ...
  describe "GET /api/handling_deficits/:id" do
    it "returns the requested handling deficit" do
      handling_deficit = create(:handling_deficit)

      get "/api/handling_deficits/#{handling_deficit.id}"
    ...
    it "returns a 404 when the handling deficit does not exist" do
      get "/api/handling_deficits/does-not-exist"
  ...
  describe "POST /api/working_sessions/:working_session_id/handling_deficits" do
    it "creates a handling deficit for the given working session" do
      working_session = create(:working_session)

      expect {
        post "/api/working_sessions/#{working_session.id}/handling_deficits",
    ...
    it "returns a 404 when the working session does not exist" do
      post "/api/working_sessions/does-not-exist/handling_deficits",
           params: { handling_deficit: { location: "trackwide", deficit: "oversteer" } }
  ...
  describe "PATCH /api/handling_deficits/:id" do
    ...
    it "returns a 404 when the handling deficit does not exist" do
      patch "/api/handling_deficits/does-not-exist", params: { handling_deficit: { deficit: "understeer" } }

      expect(response).to have_http_status(:not_found)
    end
  end

  describe "DELETE /api/handling_deficits/:id" do
    it "deletes the handling deficit" do
      handling_deficit = create(:handling_deficit)

      expect {
        delete "/api/handling_deficits/#{handling_deficit.id}"
      }.to change(HandlingDeficit, :count).by(-1)
    ...
    it "returns a 404 when the handling deficit does not exist" do
      delete "/api/handling_deficits/does-not-exist"
